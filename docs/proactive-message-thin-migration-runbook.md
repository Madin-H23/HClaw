...
| （披露）原判定「非 200」含 3xx：curl 无 `-f` 时 3xx 以 3xx 码返回，AI 会报警 | `-f` 只对 ≥400 退非 0，3xx 不失败    | **非严格等价，如实披露**：3xx 场景原版报警、登记版按成功通知；Ydisks 后台（59188）无重定向行为，该分支实际不可达，记录备查 |
...
| （披露）两条 `cmd //c` 相互独立，各拉各的、AI 分别汇报两者状态     | `&&` 链式串行：前一个 bat 退非 0 则后续全部不执行             | **失败传播条件化，如实披露**：原版两 relay 拉起互不阻断、可分别定位；登记版聚合为单一成败（首败即失败告警，具体环节查 script 运行日志） |
